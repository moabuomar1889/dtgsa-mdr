...
const roadmap = [
  {
    ...
    status: "Done",
    summary:
      "Locked stack, Prisma schema, Nocturne shell, env validation, and database foundation.",
  },
  {
    phase: "Phase 1",
    status: "Done",
    summary:
      "RBAC, masters, clients, projects, settings visibility, and onboarding flows.",
  },
  {
    phase: "Phase 2",
    status: "Active",
    summary:
      "PDI register, Excel flows, client numbering portal, MDR operations, uploads, and reply loops.",
  },
  {
    phase: "Phase 3+",
    status: "Active",
    summary:
      "Workflow, covers, transmittals, client replies, generated packages, notifications, and hardening.",
  },
] as const
...
const statusGuardrails = [
  ...
  },
  {
    ...
    detail:
      "Tracks revision lifecycle independently from workflow and client response.",
  },
  {
    title: "Client reply state",
    detail:
      "Tracks reply waiting and reply behavior without collapsing everything into one field.",
  },
] as const
    ...
    <div className="flex flex-1 flex-col gap-4 px-4 py-4 md:px-6 md:py-5">
      <section className="grid gap-4 xl:grid-cols-[1.45fr_1fr]">
        <Card className="border-line bg-panel overflow-hidden">
          <CardHeader className="border-line bg-head space-y-3 border-b">
            ...
            <div className="space-y-2">
              <CardTitle className="max-w-2xl text-[22px] font-medium tracking-[-0.02em]">
                DTGSA document control is now running on the production stack
                with live registers, workflow actions, client replies, and
                generated outbound packages.
              </CardTitle>
              <p className="text-soft max-w-3xl text-sm leading-6 md:text-base">
                The current slice covers the shell, Prisma foundation, seeded
                RBAC, masters, client creation, project onboarding, PDI, MDR,
                uploads, workflow gates, client numbering portal, replies,
                transmittals, notification plumbing, and generated PDFs while
                keeping numbering, workflow, revision, and client-response
                states isolated.
              </p>
            </div>
          </CardHeader>
            ...
            {metrics.map((metric) => (
              ...
              >
                <div className="mb-4 flex items-center justify-between">
                  <div className="bg-accent-bg text-accent-txt rounded-[10px] p-2">
                    <metric.icon className="size-5" />
                  </div>
                  <ArrowRightIcon className="text-soft size-4" />
                </div>
                <div className="space-y-1">
                  <p className="text-soft text-sm">{metric.title}</p>
                  <p className="text-xl font-semibold tracking-tight">
                    {metric.value}
                  </p>
                  <p className="text-soft text-sm leading-6">{metric.detail}</p>
                </div>
              </div>
            ))}
          </CardContent>
